...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['MEORPHIS_TEST_26_BASE_URL'].
   */
...
export class MeorphisTest26 extends Core.APIClient {
   ...
   * API Client for interfacing with the Meorphis Test 26 API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['MEORPHIS_TEST_26_API_KEY'] ?? undefined]
   * @param {Environment} [opts.environment=production] - Specifies the environment URL to use for the API.
   * @param {string} [opts.baseURL=process.env['MEORPHIS_TEST_26_BASE_URL'] ?? https://api.acme.com/v1] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = Core.readEnv('MEORPHIS_TEST_26_BASE_URL'),
    apiKey = Core.readEnv('MEORPHIS_TEST_26_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.MeorphisTest26Error(
        "The MEORPHIS_TEST_26_API_KEY environment variable is missing or empty; either provide it, or instantiate the MeorphisTest26 client with an apiKey option, like new MeorphisTest26({ apiKey: 'My API Key' }).",
      );
    }
